	...
	(function () {
		...
		function loadPlugin() {
			...
			if (initialise) {
				...
				if (showFullscreen) {
					dom.fullscreenButton = createToolbarButton(
						"fa-expand",
						function () {
				...
				if (showPause) {
					...
					dom.pauseButton.classList.add(
						"reveal-toolbar-pause-button"
					);
				...
				if (captureMenu) {
					...
					var id_menu = setInterval(function () {
						if (RevealMenu) {
							dom.menu = document.querySelector(
								"div.slide-menu-button"
							);
							if (dom.menu) {
								console.log("Moving menu button");
								dom.toolbar.insertBefore(
									dom.menu,
									dom.toolbar.firstChild
								);
				...
				var defaultStylesheet =
					document.querySelector("#toolbar-defaults");
		...
		if (!String.prototype.startsWith) {
			String.prototype.startsWith = function (searchString, position) {
				return (
					this.substr(position || 0, searchString.length) ===
					searchString
				);
			};
		}
		if (!String.prototype.endsWith) {
			String.prototype.endsWith = function (search, this_len) {
				...
				return (
					this.substring(this_len - search.length, this_len) ===
					search
				);
			};
		}
